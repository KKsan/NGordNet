...
/**
 *  This class is adopted from CS61B Proj1 Overview Slides
...
public class Plotter {
    ...
    /**
     * Creates overlaid normalized weight counts for each WORD in WORDS from
     * STARTYEAR to ENDYEAR, using NGM as a data source.
     */
    public static void plotWeightsHistory(NGramMap ngm, String[] words,
            int startYear, int endYear) {
        Chart chart = new ChartBuilder().width(800).height(600)
                .xAxisTitle("year").yAxisTitle("data").build();
        for (String word : words) {
            TimeSeries weightHistory = ngm.weightHistory(word, startYear,
                    endYear);

            chart.addSeries(word, weightHistory.years(), weightHistory.data());
        }
        new SwingWrapper(chart).displayChart();
    }
